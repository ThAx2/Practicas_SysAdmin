...
function Menu-DHCP {
    ...
    Write-Host "[OK] DHCP Configurado y Reiniciado." -ForegroundColor Green
    Pause
}
    ...
    do {
        ...
        switch ($opDNS) {
            "1" {
                ...
                Write-Host "[OK] Zona directa e inversa configuradas." -ForegroundColor Green
                Pause
            }
            "2" {
            ...
            "3" {
                Clear-Host
                Write-Host "=== INFORME GLOBAL DE REGISTROS DNS ===" -ForegroundColor Cyan
                $zonas = Get-DnsServerZone
                foreach ($z in $zonas) {
                    Write-Host "`n>> ZONA: $($z.ZoneName)" -ForegroundColor Magenta
                    Get-DnsServerResourceRecord -ZoneName $z.ZoneName | Format-Table HostName, RecordType, RecordData -AutoSize
                }
                Pause
            }
        }
