...
do {
    Clear-Host
    Write-Host "===== GESTOR FINAL (SIN ERRORES DE PARAMETROS) =====" -ForegroundColor Cyan
    ...
    if ($op -eq "1") {
        ...
        # Quitamos validación estricta para evitar el error de "not a valid DNS server"
        Set-DhcpServerv4OptionValue -ScopeId $base -OptionId 6 -Value $ip_s -ErrorAction SilentlyContinue
        
        Write-Host "`n[OK] DHCP configurado correctamente." -ForegroundColor Green
        pause
    }
    elseif ($op -eq "2") {
        $fullDomain = Read-Host "Nombre del dominio (ej: pecas.com)"
        $ip_puntos = Read-Host "IP a la que apunta (ej: 192.168.100.25)"
        ...
        # ELIMINADO -Force para evitar el error de parámetro no encontrado
        Add-DnsServerResourceRecordA -Name "@" -ZoneName $domain -IPv4Address $ip_puntos -ErrorAction SilentlyContinue
        Add-DnsServerResourceRecordA -Name "www" -ZoneName $domain -IPv4Address $ip_puntos -ErrorAction SilentlyContinue
        ...
        # 3. Registro PTR (Inverso) - Sin el parámetro -Force
        Add-DnsServerResourceRecordPtr -Name $oct[3] -ZoneName $invZone -PtrDomainName "$domain." -ErrorAction SilentlyContinue

        Write-Host "`n[OK] Configurado: $domain, www.$domain y zona inversa $invZone" -ForegroundColor Green
        pause
    }
} while ($op -ne "3")
